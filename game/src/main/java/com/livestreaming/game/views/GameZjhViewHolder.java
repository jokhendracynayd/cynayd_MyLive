...
import com.livestreaming.game.custom.GameBetCoinView;
import com.livestreaming.game.custom.PokerView;
import com.livestreaming.game.event.GameWindowChangedEvent;
import com.livestreaming.game.http.GameHttpConsts;
import com.livestreaming.game.http.GameHttpUtil;
import com.livestreaming.game.util.GameIconUtil;
import com.livestreaming.game.util.GameSoundPool;
...
public class GameZjhViewHolder extends AbsGameViewHolder {
    ...
    public GameZjhViewHolder(GameParam param, GameSoundPool gameSoundPool) {
        ...
        if (!anchor) {
            ...
            mBetMoney = 10;
            GameHttpUtil.getCoin(new HttpCallback() {
                @Override
    ...
    public void setLastCoin(String coin) {
        if (mCoinTextView != null) {
            mCoinTextView.setText(StringUtil.contact(mChargeString, " ", coin, "  "));
        }
    }
    ...
    public void handleSocket(int action, JSONObject obj) {
        ...
        switch (action) {
            case GameConsts.GAME_ACTION_OPEN_WINDOW://打开游戏窗口
                onGameWindowShow();
                break;
            case GameConsts.GAME_ACTION_CREATE://游戏被创建
                onGameCreate();
                break;
            case GameConsts.GAME_ACTION_CLOSE://主播关闭游戏
                onGameClose();
                break;
            case GameConsts.GAME_ACTION_NOTIFY_BET://开始下注
                onGameBetStart(obj);
                break;
            case GameConsts.GAME_ACTION_BROADCAST_BET://收到下注消息
                onGameBetChanged(obj);
                break;
            case GameConsts.GAME_ACTION_RESULT://收到游戏结果揭晓的的消息
                onGameResult(obj);
                break;
        }
    }
    ...
    private void playCardAnim() {
        ...
        //主播通知所有人下注
        if (mAnchor && mGameActionListener != null) {
            mGameActionListener.zjhAnchorNotifyGameBet(mLiveUid, mGameID, mGameToken, mBetTime);
        }
    }
    ...
    private void audienceBetGame(final int index) {
        GameHttpUtil.gameJinhuaBet(mGameID, mBetMoney, index, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0 && info.length > 0) {
                    JSONObject obj = JSON.parseObject(info[0]);
                    ...
                    if (mGameActionListener != null) {
                        mGameActionListener.zjhAudienceBetGame(mBetMoney, index);
                    }
                } else {
                    ToastUtil.show(msg);
                }
    ...
    private void onGameBetChanged(JSONObject obj) {
        ...
        boolean isSelf = uid.equals(CommonAppConfig.getInstance().getUid());
        if (isSelf) {//自己下的注
    ...
    public void anchorCloseGame() {
        ...
        if (mGameActionListener != null) {
            mGameActionListener.zjhAnchorCloseGame();
        }
    ...
    public void release() {
        mEnd = true;
        GameHttpUtil.cancel(GameHttpConsts.GET_COIN);
        GameHttpUtil.cancel(GameHttpConsts.GAME_JINHUA_CREATE);
        GameHttpUtil.cancel(GameHttpConsts.GAME_JINHUA_BET);
    ...
    public void onClick(View v) {
        ...
        } else if (i == R.id.score_2) {
            audienceBetGame(2);

        } else if (i == R.id.score_3) {
            audienceBetGame(3);

        } else if (i == R.id.btn_bet_shi) {
            mBetMoney = 10;
            playGameSound(GameSoundPool.GAME_SOUND_BET_CHOOSE);

        } else if (i == R.id.btn_bet_bai) {
            mBetMoney = 100;
            playGameSound(GameSoundPool.GAME_SOUND_BET_CHOOSE);

        } else if (i == R.id.btn_bet_qian) {
            mBetMoney = 1000;
            playGameSound(GameSoundPool.GAME_SOUND_BET_CHOOSE);

        } else if (i == R.id.btn_bet_wan) {
            mBetMoney = 10000;
            playGameSound(GameSoundPool.GAME_SOUND_BET_CHOOSE);
