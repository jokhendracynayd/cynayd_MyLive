...
public class LiveVoiceControlAdapter extends RefreshAdapter<LiveVoiceControlBean> {
    ...
    private final boolean mIsChatRoomTypeVideo;
    ...
    public LiveVoiceControlAdapter(Context context, List<LiveVoiceControlBean> list) {
        ...
        mColor0 = ContextCompat.getColor(context, com.livestreaming.common.R.color.blue2);
        mColor1 = ContextCompat.getColor(context, com.livestreaming.common.R.color.white);
        mIsChatRoomTypeVideo = ((LiveActivity) mContext).isChatRoomTypeVideo();
    ...
    class Vh extends RecyclerView.ViewHolder {
        ...
        void setData(LiveVoiceControlBean bean, int position, Object payload) {
            ...
            if (status == Constants.VOICE_CTRL_EMPTY || status == Constants.VOICE_CTRL_BAN) {//0 无人 2 禁麦；
                ...
                if (mIsChatRoomTypeVideo) {
                    if (mBtnControl.getVisibility() != View.INVISIBLE) {
                        mBtnControl.setVisibility(View.INVISIBLE);
                    }
                } else {
                    if (status == Constants.VOICE_CTRL_BAN) {//禁麦
                        mBtnControl.setBackground(mDrawable1);
                        mBtnControl.setTextColor(mColor1);
                        mBtnControl.setText(com.livestreaming.common.R.string.a_052);
                    } else {//0 unmanned
                        mBtnControl.setBackground(mDrawable0);
                        mBtnControl.setTextColor(mColor0);
                        mBtnControl.setText(com.livestreaming.common.R.string.a_051);
                    }
                }
            } else {//-1 关麦；1开麦 ；
                if (payload == null) {
                ...
                if (CommonAppConfig.getInstance().getUid().equals(bean.getUid())) {
                    if (mBtnDownMic.getVisibility() != View.INVISIBLE) {
                        mBtnDownMic.setVisibility(View.INVISIBLE);
                    }
                } else {
                    if (mBtnDownMic.getVisibility() != View.VISIBLE) {
                ...
                if (mBtnControl.getVisibility() != View.VISIBLE) {
                    mBtnControl.setVisibility(View.VISIBLE);
                }
                if (status == Constants.VOICE_CTRL_CLOSE) {//关麦
                    mBtnControl.setBackground(mDrawable0);
                    mBtnControl.setTextColor(mColor0);
                    mBtnControl.setText(com.livestreaming.common.R.string.a_053);
                } else {//1 开麦
                    mBtnControl.setBackground(mDrawable1);
                    mBtnControl.setTextColor(mColor1);
                    mBtnControl.setText(com.livestreaming.common.R.string.a_050);
                }
            }
