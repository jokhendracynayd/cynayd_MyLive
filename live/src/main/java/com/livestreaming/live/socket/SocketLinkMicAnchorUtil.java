...
public class SocketLinkMicAnchorUtil {
     ...
     * @param myPlayUrl 自己的播流地址
     * @param pkUid     对方主播的uid
     * @param myStream  自己的stream
     * @param toStream  对方的stream
     */
    public static void linkMicAnchorAccept(SocketClient client, String myPlayUrl, String myStream,String toStream, String pkUid) {
        if (client == null) {
            return;
        }
        UserBean u = CommonAppConfig.getInstance().getUserBean();
    ...
    public static void linkMicEnd(SocketClient client, String pkUid) {
        if (client == null) {
            return;
        }
        client.send(new SocketSendBean()
