...
public class SocketLinkMicPkUtil {
    ...
    public static void onPkBounsUpdate(SocketClient client, int value, int uid, int completed) {
        if (client == null) {
            return;
        }
        client.send(new SocketSendBean()
                .param("_method_", Constants.PK_BOUNS)
                .param("action", 14)
                .param("msgtype", 10)
                .param("uid", uid)
    ...
    public static void requestDublePk(SocketClient client, String uid) {
        client.send(new SocketSendBean()
                .param("_method_", Constants.ENTER_ROOM)
                .param("action", 1)
                .param("msgtype", 0)
                .param("roomnum", uid));
    }
