...
public class AudioRecorderEx {
    ...
    //    public static final String AUDIO_SUFFIX_WAV = ".m4a";
...
//				Log.e("amplitude","amplitude:" + result + " time:" + System.currentTimeMillis() + " show volume:" + volume);
    ...
    public File mergeAudioFile() {
        File mixFile = new File(CommonAppConfig.MUSIC_PATH, StringUtil.contact(StringUtil.generateFileName(), AUDIO_SUFFIX_MP3));
//        String path = FilePathUtil.makeFilePath(CommonAppContext.getInstance(), , "merge" + System.currentTimeMillis() + AUDIO_SUFFIX_MP3);
        String path = mixFile.getAbsolutePath();
        if (mRecordFiles.size() == 0) {
            return mixFile;
        }
//        L.e("--filePath---" + mRecordFiles.size());
        if (mixFile.exists()) {
