...
import com.livestreaming.video.views.VideoEditCutViewHolder;
import com.livestreaming.video.views.VideoEditFilterViewHolder;
import com.livestreaming.video.views.VideoEditMusicViewHolder;
import com.livestreaming.video.views.VideoMusicViewHolder;
import com.livestreaming.video.views.VideoProcessViewHolder;
...
public class VideoEditActivity extends AbsActivity implements
    ...
    public static void forward(Context context, long videoDuration, String videoPath, boolean fromRecord, MusicBean musicBean) {
        ...
        if (musicBean != null) {
            intent.putExtra(Constants.VIDEO_MUSIC_ID, musicBean);
        }
    ...
    protected void main() {
        ...
        MusicBean musicBean = intent.getParcelableExtra(Constants.VIDEO_MUSIC_ID);
        mMusicBean = musicBean;
        mHasOriginBgm = musicBean != null;
        if (mVideoDuration <= 0 || TextUtils.isEmpty(mOriginVideoPath)) {
            ToastUtil.show(WordUtil.getString(com.livestreaming.common.R.string.video_edit_status_error));
    ...
    public void onGenerateComplete(TXVideoEditConstants.TXGenerateResult result) {
        ...
        if (result.retCode == TXVideoEditConstants.GENERATE_RESULT_OK) {
            ...
            ToastUtil.show(com.livestreaming.common.R.string.video_generate_failed);
            if (mVideoGenerateViewHolder != null) {
    ...
    private void generateFinish() {
        if (mIsGenerateComplete) {
            return;
        }
        mIsGenerateComplete = true;
        L.e(TAG, "onGenerateComplete------->生成视频结束");
        if (mVideoGenerateViewHolder != null) {
            mVideoGenerateViewHolder.setProgress(100);
        }
        ToastUtil.show(com.livestreaming.common.R.string.video_generate_success);
        switch (mSaveType) {
            case Constants.VIDEO_SAVE_SAVE://仅保存
                saveGenerateVideoInfo();
                break;
            case Constants.VIDEO_SAVE_PUB://仅发布
                VideoPublishActivity.forward(mContext, mGenerateVideoPath, mGenerateVideoPathWater, mSaveType, mMusicBean != null ? mMusicBean.getId() : 0);
                break;
...
//        });
    ...
    private void startGenerateVideo() {
        ...
        if (mUseWaterMark) {
            ...
            mVideoEditer.stopPlay();
            mVideoEditer.cancel();
            mVideoGenerateViewHolder = new VideoProcessViewHolder(mContext, mRoot, WordUtil.getString(com.livestreaming.common.R.string.video_process_2));
    ...
    private void getWaterMark(final CommonCallback<Bitmap> commonCallback) {
        ...
        new DownloadUtil("waterMark").download(dir, fileName, waterMarkUrl, new FileDownloadCallback() {
            @Override
    ...
    public void onBackPressed() {
        ...
        DialogUitl.showSimpleDialog(mContext, WordUtil.getString(com.livestreaming.common.R.string.video_edit_exit), new DialogUitl.SimpleCallback() {
            @Override
    ...
    private void startPreProcess() {
        mVideoProcessViewHolder = new VideoProcessViewHolder(mContext, mRoot, WordUtil.getString(com.livestreaming.common.R.string.video_process_1));
    ...
    @Override
    public void onCancelProcessClick() {
        ToastUtil.show(WordUtil.getString(com.livestreaming.common.R.string.video_process_cancel));
        deleteOriginVideoFile();
        release();
        finish();
    }
    ...
    private void processFailed() {
        deleteOriginVideoFile();
        ToastUtil.show(com.livestreaming.common.R.string.video_process_failed);
        release();
        finish();
    }
