...
import com.livestreaming.video.utils.VideoIconUtil;
...
public class MainVideoScrollAdapter extends RecyclerView.Adapter<MainVideoScrollAdapter.Vh> {
    ...
    public MainVideoScrollAdapter(Context context, List<VideoBean> list, int curPosition) {
        mContext = context;
        mList = list;
    ...
    private void findCurVideo() {
        ...
        if (position >= 0 && mCurPosition != position) {
            ...
            if (position == 0) {
                ToastUtil.show(com.livestreaming.common.R.string.video_scroll_top);
            }
        }
    }
     ...
     * 点赞发生变化
     */
    public void onLikeChanged(String videoId, int like, String likeNum) {
