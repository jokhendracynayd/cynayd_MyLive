...
import com.livestreaming.video.activity.VideoEditActivity;
import com.livestreaming.video.custom.ColorfulProgress;
import com.livestreaming.video.custom.RangeSliderViewContainer;
import com.livestreaming.video.custom.VideoProgressController;
import com.livestreaming.video.custom.VideoProgressView;
...
public class VideoEditCutViewHolder extends AbsViewHolder implements View.OnClickListener {
    ...
    public void init() {
        ...
        sliderViewContainer.setDurationChangeListener(new RangeSliderViewContainer.OnDurationChangeListener() {
            ...
            public void onDurationChange(long startTime, long endTime) {
                if (mStartTime != null) {
                    mStartTime.setText(StringUtil.contact(String.format("%.2f", startTime / 1000f) , "s"));
                }
                if (mEndTime != null) {
                    mEndTime.setText(StringUtil.contact(String.format("%.2f", endTime / 1000f) , "s"));
                }
                if (mActionListener != null) {
        ...
        if (mEndTime != null) {
            mEndTime.setText(StringUtil.contact(String.format("%.2f", mVideoDuration / 1000f) , "s"));
        }
    }
    ...
    public void show(boolean showSpecial) {
        ...
        if (showSpecial) {
            if (mTip != null) {
                mTip.setText(com.livestreaming.common.R.string.video_edit_cut_tip_2);
            }
            ...
            if (mTip != null) {
                mTip.setText(com.livestreaming.common.R.string.video_edit_cut_tip);
            }
            if (mSpecialGroup != null && mSpecialGroup.getVisibility() == View.VISIBLE) {
